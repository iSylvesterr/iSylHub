...
function iSylHub:MakeNotify(NotifyConfig)
    ...
    NotifyConfig.Icon = NotifyConfig.Icon or "127299394628001"
    ...
    spawn(function()
        ...
        NotifyFrameReal.BackgroundColor3 = Color3.fromRGB(14, 14, 18)
...
function iSylHub:Window(GuiConfig)
	...
	LogoImg.Image = "rbxassetid://127299394628001"
    ...
    TextLabel1.Position = UDim2.new(0, 94, 0, 0)
    TextLabel1.Parent = Top
    ...
    Close.Activated:Connect(function()
        ...
        Yes.MouseButton1Click:Connect(function()
            -- Reset semua elemen saat close
            for key, element in pairs(Elements) do
                if element.Set then
                    -- Toggle: cek dari key prefix
                    if string.find(key, "^Toggle_") then
                        pcall(function()
                            if element.Value == true then
                                element:Set(false)
                            end
                        end)
                    elseif string.find(key, "^Slider_") then
                        pcall(function() element:Set(element.Default or 0) end)
                    elseif string.find(key, "^Dropdown_") then
                        pcall(function() element:Set(element.Default or "") end)
                    elseif string.find(key, "^Input_") then
